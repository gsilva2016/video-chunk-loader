...
COMMAND="start"
SOURCE_DIR=`pwd` #"$(dirname "$(dirname "$(readlink -f "$0")")")"
echo "$SOURCE_DIR"
CAMERAS=
FILES=
...
if [ "${COMMAND,,}" = "start" ]; then
    ...
    do
		  ...
		  docker run -t --rm --name camera-simulator$index --entrypoint ffmpeg --network host -v $SOURCE_DIR/sample-media:/home/pipeline-server/sample-media bluenviron/mediamtx:latest-ffmpeg -nostdin -re -stream_loop -1 -i /home/pipeline-server/sample-media/$file -c copy -f rtsp -rtsp_transport tcp rtsp://localhost:8554/camera_$index >/dev/null 2>&1 &
		  ((index+=1))
